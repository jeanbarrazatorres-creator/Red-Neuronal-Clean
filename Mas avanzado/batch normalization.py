...
def forward(X, params, trainig=True):
    ...
    cache = (
        Z1, Z1_norm, mean1, var1,
        Z2, Z2_norm, mean2, var2,
        A1, A2, mask 
    )

    return A2, cache 

def backward(X, Y, params, cache, adam, t, lr=0.01, l2=0.1):
    ...
    dmean2 = np.sum(
        DZ2_norm * -1 / np.sqrt(var2 + eps) ** (-1.5),
        axis=0,
        keepdims=True 
    ...
    DZ1_ReLU = DA1 * ReLU_deriv(Z1_norm)
    ...
    DZ1_norm = DZ1_ReLU * gamma1

    dvar1 = np.mean(
        DZ1_norm * (var1 - mean1) * -0.5 * (var1 + eps) ** (-1.5),
        axis=0,
        keepdims=True
    )

    dmean1 = np.mean(
        DZ1_norm * -1 /  np.sqrt(var1 + eps) ** (-1.5),
        axis=0,
        keepdims=True
