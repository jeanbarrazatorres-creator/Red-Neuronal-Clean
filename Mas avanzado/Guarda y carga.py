...
# ---------------- LR SCHEDULER ----------------
def lr_scheduler(initial_lr, epoch, decay=0.001):
...
for epoch in range(1,epochs+1):
    ...
    lr = lr_scheduler(initial_lr, epoch)

    Y_hat,cache = forward(X_train,params,True)
    backward(X_train,Y_train,params,cache,adam,epoch,lr=lr)
    ...
    if val_loss < best_val:
        ...
        for key in params:
            np.save(f"best_{key}.npy", params[key])
    else:
        counter += 1
    ...
    if epoch % 200 == 0:
        print(epoch, "LR:", round(lr,5), "Train:", round(train_loss,4), "Val:", round(val_loss,4))


# 🔥 CARGAR MEJOR MODELO
for key in params:
    params[key] = np.load(f"best_{key}.npy")
