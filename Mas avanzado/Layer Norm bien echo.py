...
# ---------- LayerNorm ----------
def layer_norm(Z, gamma, beta, eps=1e-8):
...
def layer_norm_backward(dout, cache):
    ...
    dvar = np.sum(
        dZ_norm*(Z-mean)*-0.5*(var+eps)**(-1.5),
        axis=1,
        keepdims=True
    )

    dmean = np.sum(
        dZ_norm*-1/np.sqrt(var+eps),
        axis=1,
        keepdims=True
...
def forward(X, params, training=True):
    ...
    Z1, cache1 = layer_norm(Z1, params["gamma1"], params["beta1"])
    A1 = ReLU(Z1)
    ...
    Z2, cache2 = layer_norm(Z2, params["gamma2"], params["beta2"])
    A2 = ReLU(Z2)

    Z3 = A2@params["W3"] + params["B3"]
    Z3, cache3 = layer_norm(Z3, params["gamma3"], params["beta3"])
    A3 = ReLU(Z3)

    Z4 = A3@params["W4"] + params["B4"]
    Z4, cache4 = layer_norm(Z4, params["gamma4"], params["beta4"])
    A4 = sigmoid(Z4)
...
def backward(X,Y,params,cache,adam,t,lr=0.01,l2=0.01):
    ...
    cache1,cache2,cache3,cache4,A1,A2,A3,A4,mask,X_input = cache

    m = X.shape[0]

    dZ4 = A4 - Y
    dZ4, dgamma4, dbeta4 = layer_norm_backward(dZ4, cache4)
    ...
    dZ3, dgamma3, dbeta3 = layer_norm_backward(dZ3, cache3)
    ...
    dZ2, dgamma2, dbeta2 = layer_norm_backward(dZ2, cache2)
    ...
    dZ1, dgamma1, dbeta1 = layer_norm_backward(dZ1, cache1)
...
for epoch in range(1,epochs+1):
    ...
    for XB,YB in get_batches(X_train,Y_train):
        Y_hat,cache = forward(XB,params,True)
        backward(XB,YB,params,cache,adam,epoch)
